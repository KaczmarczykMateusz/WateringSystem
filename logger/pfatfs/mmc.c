...
#define	FORWARD(d)	uartTxChar(d) /* Data forwarding function (Console out in this example) */
...
)
{
	...
	SPI_write((BYTE)(arg >> 24));		/* Argument[31..24] */
	SPI_write((BYTE)(arg >> 16));		/* Argument[23..16] */
	SPI_write((BYTE)(arg >> 8));			/* Argument[15..8] */
	SPI_write((BYTE)arg);				/* Argument[7..0] */
	...
	if (buff) {		/* Send data bytes */
		...
		if (sa) {	/* Initiate sector write process */
			...
			if ((SPI_receive() & 0x1F) == 0x05) {	/* Receive data resp and wait for end of write process in timeout of 500ms */
				for (bc = 5000; SPI_receive() != 0xFF && bc; bc--) _delay_us(100);	/* Wait ready */
				if (bc) res = RES_OK;
			}
			DESELECT();
			SPI_receive();
